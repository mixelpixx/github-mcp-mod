...
For large-scale file operations, use the bulk_operations toolset:

File Push Guidelines:
	- Single file: Use 'create_or_update_file'
	- Small batch (1-100 files): Use 'push_files'
	- Large batch (100+ files): Use 'push_files_chunked'

Limits:
	- push_files: max 100 files, 25MB per file, 100MB total
	- push_files_chunked: unlimited files, creates multiple commits

Chunked Push Options:
	- chunk_size: Files per commit (default: 50, max: 100)
	- continue_on_error: Continue if a chunk fails (default: false)

Use 'get_push_limits' to check current limits. Use 'bulk_delete_files' to delete multiple files atomically.`
	case "pull_requests":
...
Never:
   ...
   - Drop 'fields' param on subsequent pages if field values are needed.`
	case "repos":
		return `## Repository File Operations

File push limits apply to prevent API failures:
	- push_files: max 100 files, 25MB per file, 100MB total per push
	- For larger operations, enable the 'bulk_operations' toolset

Best practices:
	- Single file changes: Use 'create_or_update_file'
	- Multiple files (1-100): Use 'push_files'
	- Large batches (100+): Enable bulk_operations toolset and use 'push_files_chunked'`
	default:
		return ""
	}
}
